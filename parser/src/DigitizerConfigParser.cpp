...
Parser::DigitizerConfigParser::DigitizerConfigParser(std::filesystem::path path, std::vector<std::string> aTypes)
    : fPath(path), fTypes(aTypes) {}
...
std::ifstream Parser::DigitizerConfigParser::OpenFile() {
    ...
    try {
        if (!file.is_open())
            throw std::runtime_error("Cannot open config file " + fPath.string());
    } catch (const std::exception &e) {
            ...
            if (ClearString(CurStr).compare(0, key.size(), key) == 0) {
                while (getline(file, CurStr)) {
                    if (CurStr.c_str()[0] == '+') {
                        ...
                        if (ClearString(CurStr) == "Root"s)
                            writers.push_back("Root");
                        if (ClearString(CurStr) == "Txt"s)
                            writers.push_back("Txt");
                    } else
                        break;
                }
            }
            ...
            if (ClearString(CurStr).compare(0, key.size(), key) == 0) {
                while (getline(file, CurStr)) {
                    if (CurStr.c_str()[0] == '+') {
                        size_t found = CurStr.find_first_of(" ");
                        CurStr = CurStr.substr(found + 1);
                        if (ClearString(CurStr) == "qShort"s)
                            usedPar.qShort = DEFAULT_VALUE;
                        if (ClearString(CurStr) == "qLong"s)
                            usedPar.qLong = DEFAULT_VALUE;
                        if (ClearString(CurStr) == "cfd_y1"s)
                            usedPar.cfd_y1 = DEFAULT_VALUE;
                        if (ClearString(CurStr) == "cfd_y2"s)
                            usedPar.cfd_y2 = DEFAULT_VALUE;
                        if (ClearString(CurStr) == "baseline"s)
                            usedPar.baselinePSD = DEFAULT_VALUE;
                        if (ClearString(CurStr) == "height"s)
                            usedPar.height = DEFAULT_VALUE;
                        if (ClearString(CurStr) == "eventCounter"s)
                            usedPar.eventCounter = DEFAULT_VALUE;
                        if (ClearString(CurStr) == "eventCounterPSD"s)
                            usedPar.eventCounterPSD = DEFAULT_VALUE;
                        if (ClearString(CurStr) == "psdValue"s)
            ...
            if (ClearString(CurStr).compare(0, key.size(), key) == 0) {
                while (getline(file, CurStr)) {
                    if (CurStr.c_str()[0] == '+') {
                        ...
                        if (ClearString(CurStr) == "charge"s)
                            usedPar.charge = DEFAULT_VALUE;
                        if (ClearString(CurStr) == "amplitude"s)
            ...
            if (ClearString(CurStr).compare(0, key.size(), key) == 0) {
                while (getline(file, CurStr)) {
                    if (CurStr.c_str()[0] == '+') {
                        ...
                        if (ClearString(parameter) == "qShort"s)
                            hist.push_back(Global::IHist{file, ClearString(parameter), std::stoi(Nbins), std::stod(min),
                                                         std::stod(max)});
                        if (ClearString(parameter) == "qLong"s)
                            hist.push_back(Global::IHist{file, ClearString(parameter), std::stoi(Nbins), std::stod(min),
                                                         std::stod(max)});
                        if (ClearString(parameter) == "cfd_y1"s)
                            hist.push_back(Global::IHist{file, ClearString(parameter), std::stoi(Nbins), std::stod(min),
                                                         std::stod(max)});
                        if (ClearString(parameter) == "cfd_y2"s)
                            hist.push_back(Global::IHist{file, ClearString(parameter), std::stoi(Nbins), std::stod(min),
                                                         std::stod(max)});
                        if (ClearString(parameter) == "height"s)
                            hist.push_back(Global::IHist{file, ClearString(parameter), std::stoi(Nbins), std::stod(min),
                                                         std::stod(max)});
                        if (ClearString(parameter) == "eventCounter"s)
                            hist.push_back(Global::IHist{file, ClearString(parameter), std::stoi(Nbins), std::stod(min),
                                                         std::stod(max)});
                        if (ClearString(parameter) == "eventCounterPSD"s)
                            hist.push_back(Global::IHist{file, ClearString(parameter), std::stoi(Nbins), std::stod(min),
                                                         std::stod(max)});
                        if (ClearString(parameter) == "psdValue"s)
                            hist.push_back(Global::IHist{file, ClearString(parameter), std::stoi(Nbins), std::stod(min),
                                                         std::stod(max)});
                        if (ClearString(parameter) == "baseline"s)
                            hist.push_back(Global::IHist{file, ClearString(parameter), std::stoi(Nbins), std::stod(min),
                                                         std::stod(max)});
                        if (ClearString(parameter) == "charge"s)
                            hist.push_back(Global::IHist{file, ClearString(parameter), std::stoi(Nbins), std::stod(min),
                                                         std::stod(max)});
                        if (ClearString(parameter) == "amplitude"s)
                            hist.push_back(Global::IHist{file, ClearString(parameter), std::stoi(Nbins), std::stod(min),
                                                         std::stod(max)});
                        if (ClearString(parameter) == "scaler"s)
                            hist.push_back(Global::IHist{file, ClearString(parameter), std::stoi(Nbins), std::stod(min),
                                                         std::stod(max)});
                    } else
                        break;
            ...
            if (ClearString(CurStr).compare(0, key.size(), key) == 0) {
                while (getline(file, CurStr)) {
                    if (CurStr.c_str()[0] == '+') {
                        ...
                        if (ClearString(parameter) == "factor"s)
                            usedPar.factorCharge = std::stod(min);
                        if (ClearString(parameter) == "shift"s)
                            usedPar.shiftCharge = std::stod(min);
                    } else
                        break;
                }
            }
            ...
            if (ClearString(CurStr).compare(0, key.size(), key) == 0) {
                while (getline(file, CurStr)) {
                    if (CurStr.c_str()[0] == '+') {
                        ...
                        if (ClearString(parameter) == "cut"s)
                            usedPar.cut = std::stod(val);
                        if (ClearString(parameter) == "charge_range"s)
                            usedPar.chargeLimits = std::make_pair(std::stoi(val), std::stoi(val2));
                        if (ClearString(parameter) == "amplitude_range"s)
                            usedPar.amplitudeLimits = std::make_pair(std::stoi(val), std::stoi(val2));
                        if (ClearString(parameter) == "baseline"s)
                            usedPar.baselineLimits = std::make_pair(std::stoi(val), std::stoi(val2));
                        if (ClearString(parameter) == "wavelength"s)
                            usedPar.wavelength = std::stod(val);
                        if (ClearString(parameter) == "threshold"s)
                            usedPar.threshold = std::stod(val);
                    } else
                        break;
                }
            }
            ...
            if (ClearString(CurStr).compare(0, key.size(), key) == 0) {
                while (getline(file, CurStr)) {
                    if (CurStr.c_str()[0] == '+') {
                        ...
                        if (ClearString(range) == "up"s || range == "down"s)
                            usedPar.signal = ClearString(CurStr);
                        if (ClearString(range) == "factor"s)
                            usedPar.factorAmplitude = std::stod(lRange);
                        if (ClearString(range) == "shift"s)
                            usedPar.shiftAmplitude = std::stod(lRange);
                    } else
                        break;
                }
            }
