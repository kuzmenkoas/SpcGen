                ...
                while (getline(file, CurStr)) {
                    if (CurStr.c_str()[0] == '+') {
                        ...
                        if (ClearString(CurStr) == "Root"s)
                            writers.push_back("Root"s);
                        if (ClearString(CurStr) == "Txt"s)
                            writers.push_back("Txt"s);
                    } else
                        break;
                }
            }
            ...
            if (ClearString(CurStr).compare(0, key.size(), key) == 0) {
                while (getline(file, CurStr)) {
                    if (CurStr.c_str()[0] == '+') {
                        ...
                        if (ClearString(CurStr) == "waveform"s)
                            usedPar.waveform = {DEFAULT_VALUE};
                        if (ClearString(CurStr) == "scaler"s)
                            usedPar.scaler = DEFAULT_VALUE;
                        if (ClearString(CurStr) == "time"s)
                            usedPar.time = DEFAULT_VALUE;
                        if (ClearString(CurStr) == "date_time"s)
            ...
            if (ClearString(CurStr).compare(0, key.size(), key) == 0) {
                while (getline(file, CurStr)) {
                    if (CurStr.c_str()[0] == '+') {
                        ...
                        if (ClearString(parameter) == "baseline"s)
                            hist.push_back(Global::IHist{"DRS"s, ClearString(parameter), std::stoi(Nbins),
                                                         std::stod(min), std::stod(max)});
            ...
            if (ClearString(CurStr).compare(0, key.size(), key) == 0) {
                while (getline(file, CurStr)) {
                    if (CurStr.c_str()[0] == '+') {
                        ...
                        if (ClearString(range) == "up"s || ClearString(range) == "down"s)
