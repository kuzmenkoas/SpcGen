...
void Device::IDevice::Prepare() {
    ...
    parserFactory.SetThresholdFlag(bThreshold);
    if (GetConfigPath().empty())
        fParser = parserFactory.BuildParser(fDeviceType);
    else
        fParser = parserFactory.BuildParser(fDeviceType, GetConfigPath());
    if (fDigitizerTypes.empty())
        fDigitizerTypes = fParser->GetDigitizerTypes();
...
void Device::IDevice::DefineSignalDirection(std::vector<double> averageWaveform) {
    ...
    if (amplitude > baseline)
        usedParameters.signal = "up";
    else
        usedParameters.signal = "down";
}